<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		$settings = array_merge($settings,
			array('ID' => $gallery['term_id'], 'moduleName' => $gallery['name'], 'moduleUrl' => $module['url'], 'pluginUrl' => $gmCore->gmedia_url, 'libraryUrl' => $gmCore->upload['url'], 'key' => $gmGallery->options['license_key2'])
		);
		?>
		...
		var music = [
			...
			if($settings['music']){
				foreach ( $terms as $term ) {
					foreach ( $gmedia[$term->term_id] as $item ) {
						if('audio' != substr( $item->mime_type, 0, 5 )){
							continue;
						}
						$a[$item->ID]   = "		{'mid': '{$item->ID}','track': '/{$gmGallery->options['folder']['audio']}/{$item->gmuid}','title': " . json_encode( $item->title ) . ",'description': " . json_encode( str_replace(array("\r\n", "\r", "\n"), '', wpautop($item->description)) ) . "}";
					}
				}
				echo implode( ",\n", $a )."\n";
			}	?>
		];
		GmediaGallery_<?php echo $gallery['term_id']; ?> = jQuery('#GmediaGallery_<?php echo $gallery['term_id'] ?>').gmOptima([content, settings, music]);
	});
</script>
		...
		data.content = [
			...
			foreach ( $terms as $term ) {
				foreach ( $gmedia[$term->term_id] as $item ) {
					...
					$a[]   = "	{'id':'{$item->ID}','image': '/{$gmGallery->options['folder']['image']}/{$item->gmuid}','thumb': '/{$gmGallery->options['folder']['image_thumb']}/{$item->gmuid}','title': " . json_encode( $item->title ) . ",'text': " .  json_encode( str_replace(array("\r\n", "\r", "\n"), '', wpautop($item->description)) ) . ",'media': '','link': " . json_encode($item->link) . ",'linkTarget': '_self'}";
				}
			}
			echo implode( ",\n", $a )."\n"; ?>
		];
		jQuery('#GmediaGallery_<?php echo $gallery['term_id'] ?>').photobox('a', data);
	});
</script>

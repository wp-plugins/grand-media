			...
			var Container = this,
				...
				methods = {
					...
					initGallery: function(){// Init the Gallery
						...
						swfobject.embedSWF(opt.moduleUrl + '/gallery.swf', 'gmOptima_ID' + ID + '_Container', '100%', '100%', flashVerion, opt.pluginUrl + '/inc/expressInstall.swf', flashvars, parameters, attributes, methods.callbackFn);
					...
					callbackFn: function(e){// e = {(bool) success, (string) id, (reference to the active HTML object element) ref}
						if(e.success){
							...
							$('#gmOptima_ID' + ID + '_Flash', Container).css({'width': size[0], 'height': size[1]});
					...
					noFlash: function(){
						...
						$.each(Content, function(index){
							tempVar.push('<div class="gmOptima_imgContainer" tabindex="-1" id="' + Content[index].name + '">');
							$.each(this.data, function(index){
								ratio = Math.max(ratio, (this.w / this.h));
								imgdata = this;
								imgobj = new Image();
						...
						var size = responsive_size();
						$('.gmOptima_alternative', Container).css({'width': size[0], 'height': size[1]});
						$(window).resize(function(){
							size = responsive_size();
							$('.gmOptima_alternative', Container).css({'width': size[0], 'height': size[1]});
						});
						...
						tempVar = [];
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_imgContainer { background-color: ' + opt.property1.replace('0x', '#') + '; }');
						var imgDescrBg = prototypes.hexToRgb(opt.descriptionBGColor);
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_imgDescr { background-color: rgba(' + imgDescrBg.r + ',' + imgDescrBg.g + ',' + imgDescrBg.b + ',' + (opt.descriptionBGAlpha / 100) + '); color: ' + opt.imageDescriptionColor.replace('0x', '#') + '; font-size: ' + opt.descriptionFontSize + 'px; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_imgDescr .gm_title { color:  ' + opt.imageTitleColor.replace('0x', '#') + '; font-size: ' + opt.titleFontSize + 'px; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_catLinks { background-color: ' + opt.barBgColor.replace('0x', '#') + '; overflow: auto; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_catLinks a { color: ' + opt.labelColor.replace('0x', '#') + '; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_catLinks a:hover, ');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_catLinks a.active, ');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmOptima_catLinks a.active:hover { color: ' + opt.labelColorOver.replace('0x', '#') + '; }');
						...
						if(prototypes.isTouchDevice()){
							...
							swipeDiv.on('mousewheel', function(){
								...
								timer = setTimeout(function(){
									...
									//positionX = swipeWidth * times + corr;
									positionX = $('div.gmOptima_img:eq(' + times + ')', t).get(0).offsetLeft;
									var toload = $('div.gmOptima_img:eq(' + (times + Math.max(-2, (0 - times))) + ')', t).nextUntil($('div.gmOptima_img:eq(' + (times + 3) + ')', t).get(0)).andSelf().filter('.gm_thumb');
									// ? var toload = $('div.gmOptima_img:eq('+(times + Math.max(-2, (0 - times)))+')', t).nextUntil($('div.gmOptima_img:eq('+(times+3)+')', t).get(0)).addBack().filter('.gm_thumb');
									toload.each(function(){
										var curimg = $('img', this);
										methods.preloadImage(curimg, function(url){
											curimg.attr('src', url).parent().removeClass('gm_thumb');
							...
							swipeDiv.on('keydown', function(e){
								if(e.keyCode == 37 || e.keyCode == 39){
									...
									if(!$('div.gmOptima_img:eq(' + times + ')', this).length){
										return;
									}
									positionX = $('div.gmOptima_img:eq(' + times + ')', this).get(0).offsetLeft;
									var toload = $('div.gmOptima_img:eq(' + (times + Math.max(-2, (0 - times))) + ')', this).nextUntil($('div.gmOptima_img:eq(' + (times + 3) + ')', this).get(0)).andSelf().filter('.gm_thumb');

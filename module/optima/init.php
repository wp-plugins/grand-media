 ...
 * @var  $jsRun
 **/
$music = ( isset( $module_meta['music'][0] ) && !empty( $module_meta['music'][0] ) ) ? true : false;
$jsInit .= "var gmOptima_ID{$module_meta['term_id']},\n";
$jsInit .= "gmOptima_ID{$module_meta['term_id']}_Settings = {\n";
$a = array();
...
if ( isset( $module_meta['descriptionShow'][0] ) )
	$a[] = "	'descriptionShow': " . ( empty( $module_meta['descriptionShow'][0] ) ? 'false' : 'true' ) . ' /* descriptionShow */';
if ( isset( $module_meta['showDownload'][0] ) )
	$a[] = "	'showDownload': " . ( empty( $module_meta['showDownload'][0] ) ? 'false' : 'true' ) . ' /* showDownload */';
...
if ( isset( $module_meta['infoButtonText'] ) )
	$a[] = "	'infoButtonText': " . json_encode( $module_meta['infoButtonText'] ) . ' /* infoButtonText */';
if ( isset( $module_meta['slideshowText'] ) )
	$a[] = "	'slideshowText': " . json_encode( $module_meta['slideshowText'] ) . ' /* slideshowText */';
if ( isset( $module_meta['fullscreenText'] ) )
	$a[] = "	'fullscreenText': " . json_encode( $module_meta['fullscreenText'] ) . ' /* fullscreenText */';
...
$jsInit .= "gmOptima_ID{$module_meta['term_id']}_Content = [\n";
$a = array();
$crunch = array();
/**
 * @var $gMDb
...
$a = $b = array();
$jsInit .= "gmOptima_ID{$module_meta['term_id']}_Music = [";
if($music){
	foreach ( $module_meta['gMediaQuery'] as $i => $tab ) {
		$tab['mime_type'] = 'audio/mpeg';
		$gMediaQuery = $gMDb->get_gmedias( $tab );
		if ( empty( $gMediaQuery ) ) {
			continue;
		}
		...
		foreach ( $gMediaQuery as $item ) {
			$a[$item->ID]   = "\n		{'mid': '{$item->ID}','track': '/{$gmOptions['folder']['audio']}/{$item->gmuid}','title': " . json_encode( $item->title ) . ",'description': " . json_encode( str_replace(array("\r\n", "\r", "\n"), '', wpautop($item->description)) ) . "}";
		}
	}
}
if ( !empty( $a ) ) {
...
if (!empty($crunch)){
	$jsInit .= "var gmOptima_ID{$module_meta['term_id']}_Crunch = ".json_encode($crunch).";\n\n";
}
$jsRun .= "	gmOptima_ID{$module_meta['term_id']} = jQuery('#gmOptima_ID{$module_meta['term_id']}').gmOptima();\n\n";
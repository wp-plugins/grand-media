				...
				methods = {
					...
					callbackFn: function(e){// e = {(bool) success, (string) id, (reference to the active HTML object element) ref}
						if(e.success){
							...
							var bars_height = opt.thumbHeight + 30,
					...
					noFlash: function(){
						...
						$.each(Content, function(index){
							tempVar.push('<div class="gmAfflux_imgContainer" tabindex="-1" id="' + Content[index].cID + '">');
							$.each(this.data, function(index){
						...
						var size = responsive_size();
						$('.gmAfflux_alternative', Container).css({'width':size[0], 'height': size[1]});
						$(window).resize(function(){
							size = responsive_size();
							$('.gmAfflux_alternative', Container).css({'width':size[0], 'height': size[1]});
						});
						...
						tempVar = [];
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_imgContainer { background-color: ' + opt.bgColor.replace('0x', '#') + '; }');
						var imgDescrBg = prototypes.hexToRgb(opt.imageDescrBgColor);
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_imgDescr { background-color: rgba(' + imgDescrBg.r + ',' + imgDescrBg.g + ',' + imgDescrBg.b + ',' + (opt.imageDescrBgAlpha / 100) + '); color: ' + opt.imageDescrColor.replace('0x', '#') + '; font-size: ' + opt.imageDescrFontSize + 'px; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_imgDescr .gm_title { color:  ' + opt.imageTitleColor.replace('0x', '#') + '; font-size: ' + opt.imageTitleFontSize + 'px; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_catLinks { background-color: ' + opt.barsBgColor.replace('0x', '#') + '; overflow: auto; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_catLinks a { color: ' + opt.catButtonColor.replace('0x', '#') + '; }');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_catLinks a:hover, ');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_catLinks a.active, ');
						tempVar.push('div#GmediaGallery_' + ID + ' .gmAfflux_catLinks a.active:hover { color: ' + opt.catButtonColorHover.replace('0x', '#') + '; }');
						Container.append('<style type="text/css" scoped="scoped">' + tempVar.join("\n") + '</style>');
						tempVar = [];
					...
					navigationSwipe: function(){
						...
						if(prototypes.isTouchDevice()){
							var prevy, curry;
							swipeDiv.on('touchstart', function(e){
							...
							swipeDiv.on('touchmove', function(e){
								curr = e.originalEvent.touches[0].clientX;
							});

							swipeDiv.on('touchend', function(e){
								//e.preventDefault();
							...
							swipeDiv.on('mousewheel',function () {
								...
								timer = setTimeout( function(){
									...
									//positionX = swipeWidth * times + corr;
									positionX = $('div.gmAfflux_img:eq(' + times + ')', t).get(0).offsetLeft;
									var toload = $('div.gmAfflux_img:eq(' + (times + Math.max(-2, (0 - times))) + ')', t).nextUntil($('div.gmAfflux_img:eq(' + (times + 3) + ')', t).get(0)).andSelf().filter('.gm_thumb');
									// ? var toload = $('div.gmAfflux_img:eq('+(times + Math.max(-2, (0 - times)))+')', t).nextUntil($('div.gmAfflux_img:eq('+(times+3)+')', t).get(0)).addBack().filter('.gm_thumb');
									toload.each(function(){
										var curimg = $('img', this);
										methods.preloadImage(curimg, function(url){
											curimg.attr('src', url).parent().removeClass('gm_thumb');
							...
							swipeDiv.on('mousedown', function(e){
							...
							swipeDiv.on('mousemove', function(e){
								if(drag){
									e.preventDefault();
									curr = e.pageX;
									if(e.target != this){
							...
							swipeDiv.on('mouseup', function(e){
								e.preventDefault();
								$(this).off('mouseleave');
								drag = false;
								endTime = (new Date()).getTime();
							...
							swipeDiv.on('keydown', function(e){
								if (e.keyCode == 37 || e.keyCode == 39) {
									...
									if(!$('div.gmAfflux_img:eq(' + times + ')', this).length){
										return;
									}
									positionX = $('div.gmAfflux_img:eq(' + times + ')', this).get(0).offsetLeft;
									var toload = $('div.gmAfflux_img:eq(' + (times + Math.max(-2, (0 - times))) + ')', this).nextUntil($('div.gmAfflux_img:eq(' + (times + 3) + ')', this).get(0)).andSelf().filter('.gm_thumb');
